...
public class PostControllerIntegrationTest extends BaseIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        user1 = User.builder().name("User One").email("user1@example.com").password(passwordEncoder.encode("pass1")).build();
        user2 = User.builder().name("User Two").email("user2@example.com").password(passwordEncoder.encode("pass2")).build();
    ...
    void createPost_whenAuthenticated_shouldSucceed() {
        ...
        // Act
        ResponseEntity<PostDto> response = restTemplate.exchange("/api/v1/posts", HttpMethod.POST, requestEntity, PostDto.class);
    ...
    void createPost_whenNotAuthenticated_shouldFail() {
        ...
        HttpEntity<CreatePostRequestDto> requestEntity = new HttpEntity<>(requestDto); // No auth headers

        // Act
        ResponseEntity<Void> response = restTemplate.exchange("/api/v1/posts", HttpMethod.POST, requestEntity, Void.class);
    ...
    void getPost_byExistingId_shouldReturnPost() {
        ...
        Post post = postRepository.save(Post.builder()
                .title("Test Post").content("Content").author(user1).category(category1)
                .status(PostStatus.PUBLISHED).readingTime(1).latitude(0.0).longitude(0.0)
                .build());

        // Act
        ResponseEntity<PostDto> response = restTemplate.getForEntity("/api/v1/posts/" + post.getId(), PostDto.class);
    ...
    void deletePost_byAnotherUser_shouldReturn403Forbidden() {
        ...
        // Arrange
        Post postOfUser1 = postRepository.save(Post.builder()
                .title("User1 Post").content("...").author(user1).category(category1)
                .status(PostStatus.PUBLISHED).readingTime(1).latitude(0.0).longitude(0.0)
                .build());
        ...
        // Act
        ResponseEntity<Void> response = restTemplate.exchange("/api/v1/posts/" + postOfUser1.getId(), HttpMethod.DELETE, requestEntity, Void.class);
        ...
        assertThat(response.getStatusCode()).as("SECURITY ALERT: A user can delete another user's post!")
