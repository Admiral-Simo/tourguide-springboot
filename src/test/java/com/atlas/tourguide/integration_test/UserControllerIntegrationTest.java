...
public class UserControllerIntegrationTest extends BaseIntegrationTest {
    ...
    @Test
    @DisplayName("✅ [GET /user_id] Should return current user's ID when authenticated")
    void getUserId_whenAuthenticated_shouldReturnCorrectUserId() {
        // Arrange
        HttpEntity<Void> requestEntity = new HttpEntity<>(authHeaders);
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody()).isEqualTo(testUser.getId());
    }

    @Test
    @DisplayName("✅ [GET /check] Should return 200 OK when authenticated")
    void isAuthenticated_whenAuthenticated_shouldReturn200Ok() {
        // Arrange
        HttpEntity<Void> requestEntity = new HttpEntity<>(authHeaders);
